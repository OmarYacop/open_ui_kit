...
Widget _host(Widget child, {bool disableAnimations = false, bool rtl = false}) {
  return MaterialApp(
    home: MediaQuery(
...
void main() {
  ...
  testWidgets(
    'tapping the trigger opens and released actions become tappable',
    (tester) async {
      var replies = 0;
      await tester.pumpWidget(
        _host(
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        replies,
        0,
        reason: 'not yet emerged past the activation threshold',
      );
  ...
  testWidgets('collapseOnAction closes after an action fires', (tester) async {
    var archives = 0;
    await tester.pumpWidget(
      _host(
        UiContourRelease(
